...
class ResNet():
    def __init__(self, depth=101, num_classes=1000, pretrained=False, 
        ...
        if self.pretrained:
            print('=> Use pre-trained model with depth : {}'.format(self.depth))
        else:
            print('=> Create model with depth : {}'.format(self.depth))
        
        model = resnet(self.depth, self.num_classes, self.pretrained)
